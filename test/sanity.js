...
function assertReactImport(result) {
  ...
  if (match.length !== 1) {
    throw new Error(
      `more or less than one match found: ${match}\n${result.code}`
    );
  }
}
  ...
  transformFile(
    ...
    err => {
      if (err && err.message.indexOf("match case") !== -1) {
        ...
        throw new Error(
          "Test failed: Expected case sensitive error wasn‘t thrown, got: " +
            err.message
        );
      }
    }
...
transform(
  ...
  {
    presets: ["airbnb"],
    plugins: [[inlineReactSvgPlugin, { filename }]]
  },
  (err, result) => {
    if (err) throw err;
...
transformFile(
  ...
  (err1, exportResult) => {
    if (err1) throw err1;
    console.log("test/fixtures/test-export-default-as.jsx", exportResult.code);
  }
);
transformFile(
  ...
  (err1, exportResult) => {
    if (err1) throw err1;
    console.log("test/fixtures/test-export-default.jsx", exportResult.code);
  }
);
